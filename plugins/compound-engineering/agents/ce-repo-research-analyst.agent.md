...
<<<<<<< HEAD:plugins/compound-engineering/agents/research/repo-research-analyst.md
<examples>
<example>
...
=======
>>>>>>> upstream/main:plugins/compound-engineering/agents/ce-repo-research-analyst.agent.md
...
- **`mcp__auggie-mcp__codebase-retrieval`** (PRIMARY, if available): For semantic code understanding — set `directory_path` to the project root and use natural language `information_request` (e.g., "How is authentication implemented? Show middleware, models, and controllers"). Best for understanding architecture, finding patterns, and discovering related code across multiple languages.
- **Native content-search** (e.g., `Grep`): For exact text/code pattern searches with regex support (e.g., finding all references to `UserService`)
- **Native file-search/glob** (e.g., `Glob`): For file discovery by pattern (e.g., `**/*.md`, `**/AGENTS.md`)
- **Native file-read** (e.g., `Read`): For reading file contents once located
- Only use shell for commands with no native equivalent (e.g., `ast-grep`), one command at a time
