...
| Native content-search (e.g., Grep) | Finding exact identifiers, counting references, regex patterns | `class UserService`, `def process_payment` |
| Native file-search/glob (e.g., Glob) | Finding files by path pattern | `**/*.test.ts`, `app/models/*.rb` |
