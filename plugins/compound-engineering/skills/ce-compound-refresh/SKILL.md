...
For each learning in scope, read it, cross-reference its claims against the current codebase, and form a recommendation. Use `mcp__auggie-mcp__codebase-retrieval` (if available) for semantic search when verifying whether referenced patterns and approaches still exist in the codebase; fall back to native file-search/glob and content-search tools.
