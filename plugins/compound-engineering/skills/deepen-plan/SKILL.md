...
For plan sections that reference existing code or propose changes to the codebase, use codebase-retrieval (if available) to find:
- Existing implementations of similar patterns
- Current architecture in affected areas
- Related tests and their conventions

If codebase-retrieval is not available, fall back to native file-search/glob and content-search/grep tools.
