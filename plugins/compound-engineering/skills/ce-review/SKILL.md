...
Before launching parallel agents, use `mcp__auggie-mcp__codebase-retrieval` (if available) to gather context about the PR's affected areas. Search for: "Explain the architecture and patterns in [affected files/modules from PR]". Include this context in each agent's prompt for better-informed reviews. If codebase-retrieval is not available, fall back to Grep/Glob for context gathering.
